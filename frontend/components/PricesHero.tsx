...
const PricesHero: FC<{branches: any}> = ({branches}) => {
  const [form, setForm] = useState<boolean>(false);
  return (
    <div className="lg:h-screen h-max bg-primary snap-none flex items-center justify-center lg:space-x-8 space-x-0 xl:px-margin-lg lg:px-margin-base sm:px-margin-sm px-margin-xs">
        <div className="xl:w-[60%] lg:w-[45%] mt-navbar w-full flex items-center justify-center">
            ...
            <h1 className="text-tertiary absolute lg:text-left text-center xl:text-[4.2rem] md:text-[4rem] text-[3rem] font-fontExtra leading-[1.25] w-max drop-shadow-[0_4px_44px_rgba(252,92,44,0.6)]">
                OUR <a onClick={() => window.scrollTo({ top: document.getElementById('prices')!.getBoundingClientRect().top - 120, behavior: 'smooth' })} className="md:inline-block hidden group cursor-pointer rounded-full bg-secondary w-16 h-16 ml-3 outline-dashed outline-4 -outline-offset-2 hover:outline-offset-2 transition-all duration-300 outline-tertiary relative"> <img src="/statics/IndianRupee.svg" alt="" className="absolute p-1 top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 transition-all duration-300" /> </a> <br /> PRICES
                <a onClick={() => window.scrollTo({ top: document.getElementById('prices')!.getBoundingClientRect().top - 120, behavior: 'smooth' })} className="group md:hidden mx-auto block cursor-pointer rounded-full bg-secondary w-12 h-12 outline-dashed outline-2 -outline-offset-1 hover:outline-offset-2 transition-all duration-300 outline-tertiary relative"> <img src="/statics/IndianRupee.svg" alt="" className="absolute p-[0.05rem] top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 transition-all h-6 duration-300" /> </a>
            </h1>
        </div>
