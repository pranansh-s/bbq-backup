...
const FranchiseForm: FC<{show: boolean, setShow: any}> = ({show, setShow}) => {
  ...
  const lore = (e: any) => {
    ...
    setEmpty(false);
    if(!Pname || !email || !phone || !subject || !message) {
      setEmpty(true);
      return;
    }
    ...
    axios.post(('https://connect.pabbly.com/workflow/sendwebhookdata/IjU3NmMwNTY5MDYzNjA0MzM1MjZlNTUzNiI_3D_pc'), formData)
      .then(() =>{
        setPopup(true)
        setSending(false);
        setShow(false);
  ...
  return (
    <>
      ...
      <div className={`lg:w-[30rem] w-[95vw] h-max max-h-[88vh] overflow-x-hidden overflow-y-auto py-12 ${show ? 'block' : 'hidden'} lg:block lg:static fixed lg:mt-navbar mt-0 lg:top-0 lg:left-0 lg:translate-x-0 lg:translate-y-0 top-[55%] -translate-y-1/2 -translate-x-1/2 left-1/2 bg-tertiary/80 backdrop-blur-sm shadow-[0_0_60px_rgba(255,255,255,0.15)] rounded-lg text-primary text-center lg:p-8 p-6 z-30`}>
          <h4 className="font-fontBold text-3xl mb-5">Franchise Form</h4>
          ...
          <form onSubmit={lore} className="flex flex-col items-center space-y-6">
              ...
              <Input empty={empty} setEmpty={setEmpty} setState={setSubject} label="Subject" placeholder="Enter a subject" type="text" outline="black"/>
